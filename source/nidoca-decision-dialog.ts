...
export class NidocaDecisionDialog extends LitElement {
      ...
      <nidoca-dialog .show="${this.showDialog}">
        <nidoca-spacer .spacerSize="${SpacerSize.MEDIUM}" spacerAlignment="${SpacerAlignment.BOTH}">
          ...
          >
            ...
            <nidoca-typography .typographyType="${TypographyType.BODY1}" text="${this.description}"></nidoca-typography>
  ...
  private dispatchDeleteEvent() {
    BasicService.getUniqueInstance().dispatchSimpleCustomEvent(this, NidocaDecisionDialog.EVENT_OK);
  }

  private dispatchCancelEvent() {
    BasicService.getUniqueInstance().dispatchSimpleCustomEvent(this, NidocaDecisionDialog.EVENT_CANCEL);
  }
}
